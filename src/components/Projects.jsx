...
import { ArrowUpRight, Code, Cpu, Database, Network } from 'lucide-react';
...
function CropArchDiagram() {
  return (
    <svg className="w-3/5 h-3/5" viewBox="0 0 200 100">
      ...
      <circle cx="100" cy="50" r="4.5" fill="#06b6d4" />
      {/* Connector line to Docker */}
      <line x1="1045" y1="50" x2="140" y2="50" stroke="#06b6d4" strokeWidth="1.2" />
      <path d="M 104 50 L 140 50" stroke="#06b6d4" strokeWidth="1.2" />
      <path d="M 135 46 L 140 50 L 135 54" stroke="#06b6d4" strokeWidth="1.2" fill="none" />
      {/* Docker container box symbol */}
...
function TitanicArchDiagram() {
  return (
    <svg className="w-3/5 h-3/5" viewBox="0 0 200 100">
      ...
      <path d="M 90 46 L 95 50 L 90 54" stroke="#10b981" strokeWidth="1.2" fill="none" />
      {/* Decision tree split model */}
    ...
    <section id="lab" className="relative py-32 px-6 md:px-12 max-w-6xl mx-auto z-10 border-b border-white/5">
      ...
      <div className="text-left mb-16">
        ...
        <p className="text-slate-400 max-w-2xl mt-4 text-sm font-sans leading-relaxed">
          Spacious system architecture profiles. Each card projects custom glowing vector diagrams mapping ML model execution and container layers.
        </p>
      </div>
        ...
        {projectsData.map((project) => (
          ...
          >
            <div className="flex flex-col h-full justify-between text-left">
              ...
              {/* Top Vector Architecture Diagram Container */}
              <div className="relative h-40 w-full overflow-hidden rounded-xl mb-6 bg-[#080808] border border-white/5 flex items-center justify-center group-hover:border-[#06b6d4]/20 transition-all duration-500">
                <div className="absolute inset-0 opacity-15 bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:12px_12px] pointer-events-none" />
                
                {project.id === 1 && <CropArchDiagram />}
                {project.id === 2 && <ChurnArchDiagram />}
                {project.id === 3 && <TitanicArchDiagram />}
              </div>
